...
lk.LineStyle='-.';
legend({'$\mathcal{L}$','$\sqrt{A_{xx}}$','$\sqrt{A_{yy}}$','5,148s$^{-1}$'},'Location','northwest','FontSize',12);
ylabel('VWF Length' );
...
lk.LineStyle='-.';
lk.Color='b';
legend({'$\mathcal{L}$','$\sqrt{A_{xx}}$','$\sqrt{A_{yy}}$','1,032s$^{-1}$'},'Location','northwest','FontSize',12);
...
sr_half = interp1((((A(:,1)+A(:,3))/2).^0.5),sr_vec,half);
