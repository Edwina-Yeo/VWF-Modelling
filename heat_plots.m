...
sr_vec=logspace(-1,6,200)
if_mod=1
[len,A]=vwf_extension_shear(vwf_params,sr_vec); %run VWF model
...
t=tiledlayout(1,2);
nexttile
plot(sr_vec,(A(:,1)+A(:,3)+1)/3,'r'); hold on
plot(sr_vec,A(:,1),'k'); hold on
% plot(sr_vec,A(:,2),'k-.')
plot(sr_vec,A(:,3),'color',[0.5,0.5,0.5])
set(gca,'XScale','log');
set(gca,'YScale','log');
lk=xline(5e3);
ylim([5e-4,1e4])
...
lk.LineStyle='-.';
legend({'$\mathcal{L}^2$','$A_{xx}$','$A_{yy}$','5,000s$^{-1}$'},'Location','southeast','FontSize',12);
...
nexttile
plot(sr_vec_e,(A_e(:,1)+A_e(:,2)+1)/3,'r'); hold on
plot(sr_vec_e,A_e(:,1),'k','linewidth',1); hold on
% plot(sr_vec_e,zeros(size(A_e(:,1))),'k','linewidth',1); hold on

plot(sr_vec_e,A_e(:,2),'linewidth',1,'color',[0.5,0.5,0.5])
...
lk.LineStyle='-.';
lk.Color='b';
legend({'$\mathcal{L}^2$','$A_{xx}$','$A_{yy}$','5,000s$^{-1}$','850s$^{-1}$'},'Location','southeast','FontSize',12);
...
heat_plot(x,y,sqrt(ze-min(ze)),xq,yq,zpsi,DT,IO,'(c) VWF Extension','$\mathcal{E}$','E_plot',1,tdata,painters_on)
